...
| 260803-kvg | Fix Learner Preview phone/tablet device-frame rendering a large blank gray band inside the bezel, below the lesson's bottom nav, on tall desktop windows. Root cause: the device-frame `<section>` in `PreviewPage.tsx` is a flex child of `<main>` (default `align-items: stretch`), so it stretched to the full flex-row height while the iframe inside stayed capped at `min(844px, calc(100vh - 8.5rem))` — the difference was unpainted bezel area. Added `self-start` to the section so it shrink-wraps to the iframe instead; also deduped the height/minHeight literal (previously copy-pasted on the iframe and the loading placeholder) into one `frameScreenStyle(mode)` helper. Only `PreviewPage.tsx` touched — `scormExport.ts` (shared SCORM export render path) deliberately untouched. Typechecks clean, diff scoped to exactly one file. Human verification of the visual fix pending (deploying to prod for live check). | 2026-08-03 | 1945f1e | [260803-kvg-phone-preview-blank-space](./quick/260803-kvg-phone-preview-blank-space/) |
| 260814-g4m | Removed per-question correct/incorrect reveal + "Try again" gating from MCQ/True-False quiz blocks (both Learner Preview iframe and SCORM export share one render path in `scormExport.ts`) — answers now lock in silently and the learner advances freely regardless of correctness. Collapsed the pre-existing opt-in "assessment mode" branch into the only behavior, deleting the teaching/reveal branch and its dead authoring controls (module "Assessment mode" checkbox, per-block assessment selects, MCQ "Show feedback" toggle + option feedback editors, True/False feedback fields) — stored payload fields kept for revertibility. Fixed two scoring bugs: denominator is now every quiz block in the module (`data-quiz-total`, computed via new `countQuizBlocks()`) instead of only submitted questions (so skipping questions no longer inflates the score to 100%), and the per-question `cmi.core.score.raw` write that clobbered the module score was deleted — `showComplete()`'s end-of-module write is now authoritative and drives both the completion modal's "You scored X% — C of N correct" and SCORM `lesson_status` against the export dialog's pass score. Preview iframe (previously had no score UI at all) now mirrors this. 10 new vitest tests in new `scormExport.test.ts`, `pnpm test`/`typecheck`/`build`/`lint` all clean. Human verification (Learner Preview click-through + SCORM Cloud gradebook check) pending. | 2026-08-14 | e6ee835, e5a932a, 2eec9dd, f4b32e1, 03683aa | [260814-g4m-quiz-blocks-should-not-show-correct-inco](./quick/260814-g4m-quiz-blocks-should-not-show-correct-inco/) |
| 260814-ht4 | Fixed completion-screen bug where the title always read "You crushed it!" even on a failing score (screenshot: "0% — 0 of 1 correct" still showed the celebratory headline). Root causes: `showComplete()` only swapped to pass/fail copy when `completionCriteria==='passed'` (now swaps whenever the module has quiz blocks and score<passingScore, regardless of criteria — criteria still solely controls the SCORM `lesson_status` write), and Learner Preview's completion card had zero pass/fail wiring at all plus always ran on hardcoded defaults since `PreviewPage.tsx` never passed real `ExportOptions` (passingScore/completionCriteria were never persisted anywhere — lived in `ModuleEditorPage` `useState` and died with the component). Added `modules.completionSettings` (Convex schema + `setCompletionSettings` mutation) to persist passingScore/completionCriteria/completionCopy per module; new `completionCopy` lets authors override the default/pass/fail title+body (blank falls back to current hardcoded strings). Unified the two duplicated completion-card markups and JS copy-swap paths in `scormExport.ts` into shared `buildCompleteCardHtml()`/`buildCopySwapJs()`, used by both `buildLessonPage` and `buildPreviewHtml` so SCORM export and preview stay in parity. Deliberately separated `statusPassed` (SCORM status) from `copyState` (on-screen title/body/icon/confetti) so the copy-trigger fix couldn't accidentally alter the SCORM write. 22/22 vitest tests pass, typecheck/build clean, Convex schema pushed to dev. On-screen human verification of the exact reported scenario still pending. | 2026-08-14 | dd60642, 885bc63, 0f1d0bb, 4b92a43 | [260814-ht4-fix-completion-screen-pass-fail-bug-and-](./quick/260814-ht4-fix-completion-screen-pass-fail-bug-and-/) |
...
**Last activity:** 2026-08-14 - Completed quick task 260814-ht4: fixed completion-screen pass/fail copy bug (preview never showed fail state; 'completed'-criteria modules never showed it either) and made completion title/body authorable per module, persisted via new `completionSettings` Convex field (pending human verification + prod deploy).
