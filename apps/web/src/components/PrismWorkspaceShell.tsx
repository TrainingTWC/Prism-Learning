...
import { Bell, BookOpen, Brain, Layers, Palette, Search, Sparkles, Users } from 'lucide-react';
import type { ReactNode } from 'react';

type ActiveSection = 'overview' | 'modules' | 'build' | 'theme' | 'members';
...
const navItems = [
  { id: 'overview', label: 'Home', icon: BookOpen, to: '/w/$workspaceId' },
...
export function PrismWorkspaceShell({
  ...
  return (
    <div className="prism-brand-screen prism-app-shell">
      <aside className="prism-sidebar">
        <div className="border-b border-[var(--sidebar-border)] px-5 py-5">
          ...
          <p className="truncate text-lg font-bold tracking-tight text-[var(--text-primary)]">{workspaceName}</p>
          {workspaceRole && (
            <span className="badge-pill mt-3 bg-[rgba(13,140,99,0.1)] text-[var(--ember-400)]">
        ...
        <nav className="flex-1 space-y-1 px-3">
          <p className="px-3 pb-2 pt-1 text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--text-muted)]">Authoring</p>
          {navItems.map((item) => {
        ...
        <div className="border-t border-[var(--sidebar-border)] p-4">
          <div className="rounded-2xl border border-[var(--border-subtle)] bg-white/[0.025] p-4">
